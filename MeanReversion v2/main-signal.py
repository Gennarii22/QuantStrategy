        ...
        for symbol in list(self.data.keys()): 
            ...
            if total_active < self.max_positions:
                ...
                if symbol_data.CheckSetupCondition():
                    ...
                    if qty > 0:
                      # 1. Calcola i parametri esatti per l'inserimento manuale
                      stop_price_calc = symbol_data.atr.Current.Value * 3.0
                      hard_stop_level = self.Securities[symbol].Price - stop_price_calc # Stima livello stop
                      tp_target = symbol_data.bb.MiddleBand.Current.Value # Target sulla media mobile

                      # 2. Formatta il messaggio
                      subject = f"SEGNALE SWING: {symbol}"
                      message = (
                        f"SYMBOL: {symbol}\n"
                        f"ACTION: BUY LIMIT\n"
                        f"ENTRY PRICE: {round(limit_price, 2)}\n"
                        f"QUANTITY: {qty}\n"
                        f"STOP LOSS (Stima): {round(hard_stop_level, 2)}\n"
                        f"TAKE PROFIT (Target): {round(tp_target, 2)}\n"
                        f"REASON: Setup Mean Reversion (BB + CRSI)"
                      )

                      # 3. Invia Email (Assicurati di aver configurato l'email nelle impostazioni reali se live)
                      self.Notify.Email("gennaro@gpancia.com", subject, message)
    
                      # 4. Log per debug in console
                      self.Log(f"Segnale Inviato: {symbol} @ {limit_price}")
